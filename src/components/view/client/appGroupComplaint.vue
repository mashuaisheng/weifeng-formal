		...
		<div class="message-1">
            <div class="message-1-2">
                <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
                    ...
                    <el-form-item label="被投诉群组编号"  prop="toTarget">
                   ...
                   <el-form-item label="上传图片" label-width="100px" >
                        <el-upload v-model="ruleForm.uploadPicUrl" ref="uploadPicUrl" :action="domain" accept="image/png,image/gif,image/jpg,image/jpeg" list-type="picture-card" :limit="limitNum" :auto-upload="true" :on-error="uploadError" :on-success="uploadSuccess" :on-exceed="handleExceed" :before-upload="handleBeforeUpload" :on-remove="handleRemove" :file-list="fileList">
		...
		submitForm(formName){
            ...
            this.$refs[formName].validate((valid) => {
				if (!valid) return null  // 如果验证失败就不往下继续执行
                    ...
                    // this.axios.post("http://172.16.0.57:8972/app/manage/v1/complainUser?userId=" +userId +"&toTarget=" +groupId+"&userComplainImg=" +this.imagefile+"&content=" +contents+"&toTargetType="+ 2+"&token=" +localStorage.getItem("tokenlo"))
                this.axios.post("http://viefong.com:8972/app/manage/v1/complainUser?userId=" +userId +"&toTarget=" +groupId+"&userComplainImg=" +this.imagefile+"&content=" +contents+"&toTargetType="+ 2+"&token=" +localStorage.getItem("tokenlo"))
                .then(res => {
                    if(res.data.rescode==100){
                        this.$message({
                            message: '发送成功',

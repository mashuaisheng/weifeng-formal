...
XSU=({
	...
	init: function(workBook, sheetName, cell) {
		let type = typeof(workBook.Sheets[sheetName][cell])
		if (type == 'object' && !workBook.Sheets[sheetName][cell].s) {
	...
	init1: function(workBook, sheetName, cell, attr) {
		...
		let type = typeof(workBook.Sheets[sheetName][cell])
		if (type == 'object' && !workBook.Sheets[sheetName][cell].s[attr]) {
	...
	initAllCell: function(workBook, sheetName) {
		...
		for (var c = sc; c <= ec; c++) { //初始化所有单元格
			for (var r = sr; r <= er; r++) {
				...
				isExist = false;
				for (var cell in workBook.Sheets[sheetName]) {
	...
	*/
	...
	setAlignmentVertical: function(workBook, sheetName, cell, vertical) {
		...
		if (typeof(workBook.Sheets[sheetName][cell]) == 'object') {
